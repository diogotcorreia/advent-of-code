...
const isOnRemovalList = (x, y, removalList) => {
  for (let coord of removalList) if (coord.x === x && coord.y === y) return true;
  return false;
};

const removeCartAt = (x, y) => {
...
while (true) {
  ...
  if (carts.length === 1) {
    var result = carts[0].x + "," + carts[0].y;
    break;
  }
  let removalList = [];
  for (let cart of carts) {
    if (isOnRemovalList(cart.x, cart.y, removalList)) continue;
    if (cart.direction === 0) {
      if (isCart(cart.x, cart.y - 1)) removalList.push({ x: cart.x, y: cart.y - 1 });
      cart.y--;
      ...
      if (isCart(cart.x + 1, cart.y)) removalList.push({ x: cart.x + 1, y: cart.y });
      cart.x++;
      let tile = track[cart.y][cart.x];
      if (tile === 2) {
        // 90 DEG (/)
  ...
  for (let coord of removalList) removeCartAt(coord.x, coord.y);
}

console.log("The answer is:");
console.log(result);
console.timeEnd("13-part2");
